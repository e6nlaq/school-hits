...
describe("integer", () => {
	...
	test.concurrent("float", () => {
		for (let value = -1000; value <= 1000; value += 0.1) {
			expect(input_format(value, -Infinity, Infinity, true)).toBe(Math.round(value));
		}
	});
});
...
describe("integer_random", () => {
	...
	test.concurrent("value", () => {
		for (let i = 0; i < random_count; ++i) {
			const value = random.integer(Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
			expect(input_format(value, -Infinity, Infinity, true)).toBe(value);
		}
	});
	...
	test.concurrent("float", () => {
		for (let i = 0; i < random_count; ++i) {
			const value = random.real(Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
			expect(input_format(value, -Infinity, Infinity, true)).toBe(Math.round(value));
		}
	});
});
